...
# AGomez, 2021-Dec-12, Modified code to handle file names for save_inventory and load_inventory
#------------------------------------------#
...
class FileIO:
    ...
    def save_inventory(file_name: list, lst_Inventory: list) -> None:
        ...
        """
        file_name_CD_Inventory = file_name[0]
        file_name_Track_Inventory = file_name[1] 
        try:
            with open(file_name_CD_Inventory, 'w') as file:
                for disc in lst_Inventory:
                    file.write(disc.get_record())
            with open(file_name_Track_Inventory, 'w') as file:
                for disc in lst_Inventory:
    ...
    def load_inventory(file_name: list) -> list:
        ...
        file_name_Track_Inventory = file_name[1] 
        try:
            with open(file_name_CD_Inventory, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
                    row = DC.CD(data[0], data[1], data[2])
                    lst_Inventory.append(row)
            with open(file_name_Track_Inventory, 'r') as file:
                for line in file:
                    data = line.strip().split(',')
